<template>
  <div class="main">
    <div class="air-column">
      <h2>乘机人</h2>
      <el-form class="member-info">
        <div class="member-info-item" v-for="(item,index) in users" :key="index">
          <el-form-item label="乘机人类型">
            <el-input placeholder="姓名" class="input-with-select" v-model="users[0].username">
              <el-select slot="prepend" value="1" placeholder="请选择">
                <el-option label="成人" value="1"></el-option>
              </el-select>
            </el-input>
          </el-form-item>

          <el-form-item label="证件类型">
            <el-input placeholder="证件号码" class="input-with-select" v-model="users[0].id">
              <el-select slot="prepend" value="1" placeholder="请选择">
                <el-option label="身份证" value="1" :checked="true"></el-option>
              </el-select>
            </el-input>
          </el-form-item>

          <span class="delete-user" @click="handleDeleteUser(index)">-</span>
        </div>
      </el-form>

      <el-button class="add-member" type="primary" @click="handleAddUsers">添加乘机人</el-button>
    </div>

    <div class="air-column">
      <h2>保险</h2>
      <div>
        <div class="insurance-item" v-for="(item,index) in insurances" :key="index">
          <el-checkbox
            :label="`${item.type}：￥${item.price}/份×1  
          最高赔付${item.compensation}`"
            border
            @change="heihei(item.id)"
          ></el-checkbox>
        </div>
      </div>
    </div>

    <div class="air-column">
      <h2>联系人</h2>
      <div class="contact">
        <el-form label-width="60px">
          <el-form-item label="姓名">
            <el-input v-model="contactName"></el-input>
          </el-form-item>

          <el-form-item label="手机">
            <el-input placeholder="请输入内容" v-model="contactPhone">
              <template slot="append">
                <el-button @click="handleSendCaptcha">发送验证码</el-button>
              </template>
            </el-input>
          </el-form-item>

          <el-form-item label="验证码">
            <el-input v-model="captcha"></el-input>
          </el-form-item>
        </el-form>
        <el-button type="warning" class="submit" @click="handleSubmit">提交订单</el-button>
      </div>
    </div>
     <!-- 模板中引用总价格触发计算 -->
        <span v-show="false">{{getallprice}}</span>
  </div>
</template>

<script>
export default {
  data() {
    return {
      users: [{ username: "", id: "" }],
      insurances: [],
      contactName: "",
      contactPhone: "13312882474",
      invoice: false,
      seat_xid: "",
      air: "",
      captcha: "000000",
      //   机票数据
      airinfo: {},
      // 操作保险数据
      infoData:[], 
    
    };
  },
  methods: {
    // 添加乘机人
    handleAddUsers() {
      this.users = [...this.users, { username: "", id: "" }];
    },

    // 移除乘机人
    handleDeleteUser(index) {
      this.users.splice(index, 1);
    },

    // 发送手机验证码
    handleSendCaptcha() {
      if (!this.contactPhone) {
        this.$message.error("电话不能为空");
        return;
      }
      this.$axios({
        url: "/captchas",
        method: "POST",
        data: { tel: this.contactPhone }
      }).then(res => {
        console.log(res);
        const { code } = res.data;
        this.$alert(`手机号码验证码是:${code}`, "提示");
      });
    },
    // 选择保险--判断是否选了，看里面有多少个Id
    heihei(id) {
      //   console.log(5555);
      const index = this.infoData.indexOf(id);
      if (index > -1) {
        this.infoData.splice(index, 1);
      } else {
        this.infoData.push(id);
      }
      // console.log(this.infoData);
    },
    // 提交订单
    handleSubmit() {
        const data ={
            users:this.users,
            insurances:this.airinfo,
            contactName:this.contactName,
            contactPhone:this.contactPhone,
            invoice:this.invoice,
            captcha:this.captcha,
            seat_xid:this.$route.query.seat_xid,
            air:this.$route.query.id
        }
        // console.log(data)
         // 在提交之前判断非空
      if(!this.users[0].username || !this.users[0].id){
        this.$message.error('乘机人信息为空噢')
        return false
      }
      if(!this.contactName){
        this.$message.error('联系人信息为空噢')
        return false
      }
      if(!this.contactPhone){
        this.$message.error('联系人号码为空噢')
        return false
      }
       if(!this.captcha){
        this.$message.error('验证码为空噢')
        return false
      }
      this.$message({
        message:'订单收到，请稍等',
        type:'success'
      })
        this.$axios({
            url:'/airorders',
            headers:{Authorization:`Bearer ${this.$store.state.user.userInfo.token}`},
            method:'POST',
            data
        }).then(res=>{
            // console.log(res.data.data.air,222)
            const {id}=res.data.data
            // console.log(id,777)
            console.log(res.data.data.id)
            this.$router.push({
              path:'/air/pay',
              query:{id}
            })
        })
    }
  },
  mounted() {
    this.$axios({
      url: "/airs/" + this.$route.query.id,
      params: {
        seat_xid: this.$route.query.seat_xid
      }
    }).then(res => {
      console.log(res, 111);
      const { insurances } = res.data;
      this.insurances = insurances;
      this.airinfo=res.data
      this.$store.commit('air/setInfoData',this.airinfo)
      //  console.log(this.airinfo,888)
    // console.log(this.infoData)
    });
  },
  computed:{
  //  子组件之间传值
  getallprice(){
     if(!this.airinfo.seat_infos){
                return 0;
            }
              let price=0
    price += this.airinfo.seat_infos.org_settle_price;
    price += this.airinfo.airport_tax_audlet;
    price += 30*this.infoData.length;
    price *= this.users.length
    this.$store.commit('air/getallprice',price)
    return price;
  }
  }
};
</script>

<style scoped lang="less">
.air-column {
  border-bottom: 1px #ddd dashed;
  padding-bottom: 20px;
  margin-bottom: 20px;
}

.air-column h2 {
  margin-bottom: 20px;
  font-size: 22px;
  font-weight: normal;
}

/deep/ .el-select .el-input {
  width: 130px;
}

.input-with-select {
  width: 590px;
}

.input-with-select /deep/ .el-input-group__prepend {
  background-color: #fff;
}
.member-info /deep/ .el-form-item {
  margin-bottom: 0;
}

.member-info-item {
  border-bottom: 1px #eee dashed;
  padding-bottom: 20px;
  position: relative;

  &:first-child {
    .delete-user {
      display: none;
    }
  }
}

.add-member {
  margin-top: 20px;
}

.delete-user {
  display: block;
  background: #ddd;
  width: 16px;
  height: 16px;
  font-size: 14px;
  text-align: center;
  line-height: 16px;
  color: #fff;
  cursor: pointer;
  border-radius: 50px;
  position: absolute;
  right: -30px;
  top: 50%;
}

.insurance {
  > div {
    margin-top: 10px;
  }
}

.insurance-item {
  margin-bottom: 20px;
}

.contact {
  /deep/ .el-input {
    width: 50%;
  }
}

.submit {
  margin: 50px auto;
  display: block;
  width: 250px;
  height: 50px;
}
</style>